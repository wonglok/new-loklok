...
export const makeIceCream = async ({ api, scene, parent, cubeTexture }) => {
  let rID = getID()
  // eslint-disable-next-line
  let icecreamlink = require('file-loader!../Model/ice-cream/ice-cream.glb')
  let icecreamScene = await loadGLB({ file: icecreamlink })
  ...
  parent.add(icecreamScene)
  api.teardown[rID] = () => {
  }
  api.tasks[rID] = () => {
    let time = window.performance.now() * 0.001
    icecreamScene.rotation.y = time
    // let time = window.performance.now() * 0.001
    // console.log(time)
  }
}
...
export const setupBase = async ({ api, mounter, vm }) => {
  ...
  makeIceCream({ ...env, scene, camera, parent: parent, cubeTexture: canvasCubeTexture })
