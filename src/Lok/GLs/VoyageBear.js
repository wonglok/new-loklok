...
export const makeCanvasCubeTexture = async ({ api }) => {
  ...
  class TouchTexture {
    ...
    clear () {
      // this.ctx.fillStyle = `hsla(339, 71%, ${51 + 20}%, 0.5)`
    ...
    drawPoint (point) {
      ...
      // color = `${(((intensity * 0.1)) * 360).toFixed(0)}, ${70}%, 60%`
      ...
      ctx.shadowColor = `hsla(339, 71%, 51%, ${0.35 * intensity})` // (default transparent black)
...
export const makeLogo = async ({ cubeTexture, parent, idx = 0 }) => {
  ...
  let logos = [
    await loadTexture(require('../Textures/demos/voyage-bear.png'))
